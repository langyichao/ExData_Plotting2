...
# get all motor vehicles related SCC's
...
# get only motor vehicles related emissions for Baltimore City
motor <- subset(blt, SCC %in% motor.scc)
motor$year <- as.factor(motor$year)

# overall change
g1 <- qplot(year, Emissions, data=motor, geom='bar',stat = 'identity')+
        ylab('Emissions, tons')

# density lines
g2 <- qplot(Emissions,data=motor,col=year,geom='density', log='x') + 
...
f <- grid.arrange(arrangeGrob(g1,g3, ncol=2),g2,ncol=1,
                  ...
                  sub = paste('Source: Baltimore City, the Environmental Protection Agency,', 
